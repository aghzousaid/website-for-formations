...
<?php
  ...
  if (isset($_POST['upload'])) {
    ...
    $description= mysqli_real_escape_string($db, $_POST['description']);
  	...
  	$sql = "INSERT INTO mixtes (theme, profession, region, lat, long, titre, ville, dat, adresse, code_postale, presentiel, 
      non_presentiel, description, competence, duree_presentiel, duree_non_presentiel, image) VALUES ($theme, $public, $region, $lat, $long,
       $titre, $ville, $date, $adresse, $code_postale, $deroule_presentiel,$deroule_non_presentiel, $description, 
       $competenceb, $duree_presentiel, $duree_non_presentiel, $image)";
      // execute query
  ...
  <?php
    while ($row = mysqli_fetch_array($result)) {
      echo "<div id='img_div'>";
        ...
        print_r(str_word_count($row['profession'], 1));
        echo htmlEntities(str_word_count($row['profession']));
      echo "</div>";
    }
  ?>
